...
void registrarEntrada(Participante vetor[], int n)
{
    ...
    struct tm dataHora = pegaDataHora();
    ofstream procuradorArquivo;
    ...
    procuradorArquivo << matricula << ";" << dataHora.tm_mday << "/" << dataHora.tm_mon + 1 << "/" << dataHora.tm_year + 1900 << " " << dataHora.tm_hour << ":" << dataHora.tm_min << ":" << dataHora.tm_sec << "\n";
    procuradorArquivo.close();
    cout << "Entrada registrada com sucesso\n";
}

void registrarSaida(Participante vetor[], int n)
{
    cout << "Registrar saída\n";
    string matricula;
    ...
    struct tm dataHora = pegaDataHora();
    ofstream procuradorArquivo;
    procuradorArquivo.open("saida.csv", ios::out | ios::app);
    procuradorArquivo << matricula << ";" << dataHora.tm_mday << "/" << dataHora.tm_mon + 1 << "/" << dataHora.tm_year + 1900 << " " << dataHora.tm_hour << ":" << dataHora.tm_min << ":" << dataHora.tm_sec << "\n";
    procuradorArquivo.close();
...
void menu(Participante vetor[], int n, int totalInscritos)
{
    ...
    do
    {
        ...
        case 2:
            ...
            break;
        case 3:
            registrarEntrada(vetor, n);
            break;
        case 4:
            registrarSaida(vetor, n);
            break;
        case 5:
            exit(1);
        default:
